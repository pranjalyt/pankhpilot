        ...
        <section className="py-20 bg-[var(--bg-primary)] transition-colors duration-200">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex justify-between items-end mb-12">
                    <div>
                        <h2 className="text-3xl md:text-4xl font-bold text-[var(--text-primary)] mb-2">OUR COURSES</h2>
                        <p className="text-[var(--text-secondary)]">Start your journey with our specialized programs.</p>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                    {featured.map((course) => (
                        <div key={course.id} className="bg-[var(--card-bg)] rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all group flex flex-col h-full border border-[var(--border)]">
                            ...
                            <div className="p-6 flex-grow flex flex-col">
                                <p className="text-[var(--text-secondary)] text-sm mb-4 line-clamp-3 flex-grow">{course.description}</p>
                                <div className="space-y-4">
                                    <div className="flex flex-wrap gap-2">
                                        {course.tags.slice(0, 2).map(tag => (
                                            <span key={tag} className="px-2 py-1 bg-[var(--surface-bg)] text-xs font-medium text-[var(--text-primary)] rounded-md">
                                                {tag}
                                            </span>
                                        ))}
                                    </div>
                                    <div className="flex items-center justify-between pt-4 border-t border-[var(--border)]">
                                        <span className="text-[var(--text-primary)] font-bold">{course.price}</span>
                                        <Link
